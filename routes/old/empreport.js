...
app.post('/updateAbsent', (req, res, next) => {
    Employee.findById({ _id:req.body.id }, (err, emp) => {
        let ttl_days = 30;
        ...
        if ( emp.emp_pay_method ==='PF' && req.body.absent > 15 ) {
        ...
        } else if( emp.emp_pay_method ==='PF' && req.body.absent <= 15 ) {
...
app.post('/updatePfamt', (req, res) => {
    Employee.findById({ _id:req.body.id }, (err, emp) => {
        let salary = Math.round(emp.emp_gr_value - req.body.pf_amt);
        Employee.updateOne({ _id:req.body.id }, {
            $set: { 
...
app.post('/updateEsiamt', (req, res) => {
    Employee.findById({ _id:req.body.id }, (err, emp) => {
        let salary = Math.round(emp.emp_gr_value - req.body.esi_amt);
        Employee.updateOne({ _id:req.body.id }, {
            $set: { 
                    "emp_esi_amt": req.body.esi_amt,
                    "emp_gr_total": salary 
                }
        }).then(Employee.find().then( data => {
            Usr.find({ usr_name: req.session.user }, (err, usr) => {
